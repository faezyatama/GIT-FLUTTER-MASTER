...
class DataPenggunaAgen extends StatefulWidget {
  @override
  _DataPenggunaAgenState createState() => _DataPenggunaAgenState();
}

class _DataPenggunaAgenState extends State<DataPenggunaAgen> {
  ...
  loadDataAgen(halaman) async {
    ...
    var grup = c.pilihanDetailPelanggan.value;

    var datarequest =
        '{"pid":"$pid","skip":"$halaman","nama":"$nama","grup":"$grup"}';
    var bytes = utf8.encode(datarequest + '$token');
    var signature = md5.convert(bytes).toString();
    var user = dbbox.get('loginSebagai');

    var url = Uri.parse('${c.baseURL}/agensi/dataAgenPengguna');
