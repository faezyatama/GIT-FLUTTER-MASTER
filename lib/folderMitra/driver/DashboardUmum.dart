...
class DashboardDriverUmum extends StatefulWidget {
  @override
  _DashboardDriverUmumState createState() => _DashboardDriverUmumState();
}

class _DashboardDriverUmumState extends State<DashboardDriverUmum> {
  ...
  Widget build(BuildContext context) {
    return (Scaffold(
        appBar: AppBar(
          title: Text('Driver ' + c.namaAplikasi),
                    ...
                    GestureDetector(
                      ...
                      child: Column(
                        children: [
                          Badge(
                            showBadge:
                                (c.antaranIntegrasi.value > 0) ? true : false,
                            badgeContent: Text(
                          ...
                          Text('Kurir', style: TextStyle(fontSize: 11))
                        ],
                      ),
                    ),
                    GestureDetector(
                      ...
                      child: Column(
                        children: [
                          Badge(
                            showBadge: (c.antaranOjek.value > 0) ? true : false,
                            badgeContent: Text(c.antaranOjek.value.toString(),
                                style: TextStyle(color: Colors.white)),
                            child: Image.asset(
                              'images/iconmitra/UmumOrder.png',
                              width: Get.width * 0.15,
                          ...
                          Text('Pengantaran', style: TextStyle(fontSize: 11))
                        ],
                      ),
                    ),
                    GestureDetector(
  ...
  void onlineOffline() async {
    ...
    //print(response.body);
    // print('Proses Online atau Offline');
  ...
  cekOrderBerlangsung() async {
    ...
    // EasyLoading.dismiss();
    // print(response.body);
    // print('cekOrderBerlangsung');
    if (response.statusCode == 200) {
      Map<String, dynamic> hasil = jsonDecode(response.body);
      if (hasil['status'] == 'success') {
        c.kodeJualPengantaranDriver.value = hasil['orderTerbaru'];
  ...
  cekOrderOjekBerlangsung() async {
    ...
    var url = Uri.parse(
        '${c.baseURLdriver}/mobileAppsMitraDriver/AntaranOjekStatusUmum');
    ...
    if (response.statusCode == 200) {
      Map<String, dynamic> hasil = jsonDecode(response.body);
      if (hasil['status'] == 'success') {
        c.antaranOjek.value = hasil['jumlahOrder'];
        c.btnOnlineOffline.value = hasil['onoff'];
      }
    }
  }
