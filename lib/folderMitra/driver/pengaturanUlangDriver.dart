...
class AturUlangDriver extends StatefulWidget {
  @override
  _AturUlangDriverState createState() => _AturUlangDriverState();
}

class _AturUlangDriverState extends State<AturUlangDriver> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Atur Ulang Data Driver"),
      ...
      body: Container(
        ...
        child: ListView(
          children: [
            ...
            Text(
              'Lengkapi dan kirim berkas kamu, Setelah peninjauan dari ${c.namaAplikasi} maka Akun Driver kamu akan segera aktif',
              style: TextStyle(
  ...
  void prosesPendaftaranDriver() async {
    ...
    if (response.statusCode == 200) {
      ...
      if (hasil['status'] == 'sukses integrasi') {
        ...
        Get.back();
        Get.back();
        Get.to(() => DashboardIntegrasi());
      } else if (hasil['status'] == 'waiting') {
        Get.back();
        Get.back();
        Get.back();
        Get.to(() => WaitingRegister());
      } else {
        Get.back();
      }
    }
