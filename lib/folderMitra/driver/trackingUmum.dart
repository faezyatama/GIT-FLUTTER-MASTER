...
class PetaKurirUmum extends StatefulWidget {
  @override
  _PetaKurirUmumState createState() => _PetaKurirUmumState();
}

class _PetaKurirUmumState extends State<PetaKurirUmum> {
  ...
  void initState() {
    ...
    polylinePoints = PolylinePoints();
    timer =
        Timer.periodic(Duration(seconds: 8), (Timer t) => cekLokasidanOutlet());
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: (AppBar(
        title: Text('Peta Pengantaran Paket'),
  ...
  cekLokasidanOutlet() async {
    ...
    if (cekin == true) {
      ...
      var kodetrx = c.kodeTransaksiKurirUmum.value;
      ...
      var url = Uri.parse(
          '${c.baseURLdriver}/mobileAppsMitraDriver/showmapKurirUmum');
      ...
      if (response.statusCode == 200) {
        ...
        if (hasil['status'] == 'success') {
          ...
          //DRIVER ASUMSI PENGGUNA
          iddriver.value = hasil['idChatPemesan'];
          if (iddriver.value == '0') {
            // cekDriverDisekitar();
          } else {
            timer?.cancel();
            setState(() {
              var detDriver = hasil['detailPemesan'];
              gambarDriver.value = detDriver[3];
              namaDriver.value = detDriver[0];
              hp.value = detDriver[2];
              platNomorDriver.value = detDriver[1];

              idChatDriver = hasil['idChatPemesan'];
