...
class DetailBelanjaanMP extends StatefulWidget {
  DetailBelanjaanMP({this.kodePesanan});
  final String kodePesanan;

  @override
  _DetailBelanjaanMPState createState() => _DetailBelanjaanMPState();
}

class _DetailBelanjaanMPState extends State<DetailBelanjaanMP> {
  ...
  rekapBelanjadanAlamat() async {
    ...
    var url = Uri.parse('${c.baseURLmp}/mobileAppsOutlet/detailBelanjaanMP');
  ...
  void terimaOrderIni(String kodetrx) async {
    ...
    var url = Uri.parse('${c.baseURLmp}/mobileAppsOutlet/terimaOrderMP');
    ...
    if (response.statusCode == 200) {
      ...
      if (hasil['status'] == 'success') {
        ...
        c.stMasukMP.value = hasil['masuk'];
        c.stProsesMP.value = hasil['proses'];
        c.stKirimMP.value = hasil['kirim'];
        if (mounted) {
          AwesomeDialog(
            context: Get.context,
            ...
            btnOkText: 'OK SIAP',
            btnOkOnPress: () {
              c.selectedIndexOrderMPKU.value = 1;
              Get.back();
              Get.back();
              Get.to(() => PesananMP());
            },
  ...
  void tolakOrderIni(String kodePesanan) async {
    ...
    var url = Uri.parse('${c.baseURLmp}/mobileAppsOutlet/tolakOrderMP');
