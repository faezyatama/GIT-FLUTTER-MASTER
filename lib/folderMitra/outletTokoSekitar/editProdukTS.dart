...
class EditProdukTS extends StatefulWidget {
  @override
  _EditProdukTSState createState() => _EditProdukTSState();
}

class _EditProdukTSState extends State<EditProdukTS> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
            title: Text('Edit Produk Toko'), backgroundColor: Colors.green),
  ...
  void prosesUpdateProduk() async {
    ...
    var itemid = c.itemEditPilihanTSKU.value;
    var user = dbbox.get('loginSebagai');

    var datarequest =
        '{"pid":"$pid","hppModal":"$hppModal","bcode":"$bcode","itemid":"$itemid","pic6":"$pic6","pic5":"$pic5","pic4":"$pic4","pic3":"$pic3","pic2":"$pic2","pic1":"$pic1","pos":"$pos","jet":"$jet","sicepat":"$sicepat","jnt":"$jnt","jne":"$jne","intg":"$intg","ssik":"$ssik","namaProduk":"$namaProduk","des":"$des","kategori":"$kate","sub":"$sub","spesifik":"$spesifik","berat":"$berat","harga":"$harga","stok":"$stok","kondisi":"$kon"}';
    var bytes = utf8.encode(datarequest + '$token' + user);
    var signature = md5.convert(bytes).toString();

    var url =
        Uri.parse('${c.baseURLtokosekitar}/mobileAppsOutlet/updateProdukTS');
  ...
  void bukaDataProduk() async {
    ...
    var url = Uri.parse(
        '${c.baseURLtokosekitar}/mobileAppsOutlet/dataItemUntukEditTS');
    ...
    if (response.statusCode == 200) {
      ...
      if (hasil['status'] == 'success') {
        ...
        ctrlBarcode.text = hasil['barcode'];
        ctrlHargaModal.text = hasil['hppOutlet'].toString();
