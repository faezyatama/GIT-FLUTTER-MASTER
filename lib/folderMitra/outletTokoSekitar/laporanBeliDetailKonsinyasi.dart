...
class LaporanDetailKonsinyasi extends StatefulWidget {
  @override
  _LaporanDetailKonsinyasiState createState() =>
      _LaporanDetailKonsinyasiState();
}

class _LaporanDetailKonsinyasiState extends State<LaporanDetailKonsinyasi> {
  ...
  final ctrlRetur = TextEditingController();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Detail Pembelian Konsinyasi'),
  ...
  cekLaporan() async {
    ...
    var url = Uri.parse(
        '${c.baseURLtokosekitar}/mobileAppsOutlet/laporanPembelianKonsinyasiDetail');
      ...
      order.add(Container(
        child: GestureDetector(
          onTap: () {
            ...
            var nama = ord[3];
            var jumlah = ord[0];
            var sisa = 'Stok saat ini : ${ord[4]}';
            var total = ord[2];
            stokP = ord[4];

            if (stokP == 0) {
              Get.snackbar('Proses Retur',
                  'Proses retur tidak dapat dilanjutkan stok 0');
            } else {
              returPengembalianBarang(id, nama, jumlah, sisa, total);
            }
          },
          child: Card(
              elevation: 0,
                        ...
                        IconButton(
                            onPressed: () {
                              ...
                              if (stokP == 0) {
                                ...
                                returPengembalianBarang(
                                    id, nama, jumlah, sisa, total);
                              }
                            },
                            icon: Icon(
                              Icons.pending,
      ...
      body: Container(
        child: Column(
          children: [
            ...
            Text(
              'Pembayaran Pembelian Konsinyasi',
  ...
  void returPengembalianBarang(id, nama, jumlah, sisa, total) {
    AwesomeDialog(
      context: Get.context,
      dialogType: DialogType.noHeader,
      animType: AnimType.rightSlide,
        ...
        child: Column(
          children: [
            ...
            Container(
              ...
              child: TextField(
                onChanged: (ss) {
                  if (int.parse(ss) > stokP) {
                    ctrlRetur.text = stokP.toString();
                    Get.snackbar('Kesalahan..!',
                        'kamu memasukan maksimal produk yang bisa di retur');
                  }
                },
                style: TextStyle(
                    fontWeight: FontWeight.w500,
                ...
                decoration: InputDecoration(
                    labelText: 'Jumlah yang di retur',
                    border: OutlineInputBorder(
                        borderRadius: BorderRadius.circular(10))),
              ),
            ),
      ...
      btnOkColor: Colors.green,
      btnOkText: 'Proses retur barang',
      btnOkOnPress: () {
        if ((ctrlRetur.text == '') || (ctrlRetur.text == '0')) {
          Get.snackbar('Kesalahan', 'Masukan jumlah barang yang akan diretur');
        } else {
          kirimReturBarang(id);
        }
      },
  ...
  void kirimReturBarang(id) async {
    bool conn = await cekInternet();
    if (!conn) {
      return noInternetConnection();
    }
    ...
    var pid = dbbox.get('person_id');
    var kodeTrx = c.kodePembelianTokoSekitar.value;
    var jumretur = ctrlRetur.text;
    var user = dbbox.get('loginSebagai');

    var datarequest =
        '{"pid":"$pid","kodetrx":"$kodeTrx","idBarang":"$id","jumretur":"$jumretur"}';
    var bytes = utf8.encode(datarequest + '$token' + user);
    var signature = md5.convert(bytes).toString();

    var url = Uri.parse(
        '${c.baseURLtokosekitar}/mobileAppsOutlet/returBarangKonsinyasi');
    ...
    if (response.statusCode == 200) {
      Map<String, dynamic> hasil = jsonDecode(response.body);
      if (hasil['status'] == 'success') {
        Get.snackbar('Proses Berhasil', 'Proses retur berhasil dilakukan');
