...
class PembayaranFreshmart extends StatefulWidget {
  @override
  _PembayaranFreshmartState createState() => _PembayaranFreshmartState();
}

class _PembayaranFreshmartState extends State<PembayaranFreshmart> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Pembayaran Freshmart'),
                  ...
                  children: [
                    ...
                    Column(
                      ...
                      children: [
                        ...
                        Text('${c.jumlahItemFm.value} Item pesanan',
                  ...
                  children: [
                    ...
                    Column(
                      ...
                      children: [
                        ...
                        Text(' Pengiriman : ${c.pilihanKurirFm.value}',
                            style: TextStyle(fontSize: 11, color: Warna.grey))
                      ],
                    ),
                  ],
  ...
  pilihMetodeBayar(String pilih, newVal) {
    if (pilih == 'saldo') {
      if (c.saldoInt.value >= c.hargaKeranjangFm.value) {
        swCod.value = !newVal;
  ...
  kirimBelanjaNowSaldo() async {
    ...
    var token = dbbox.get('token');
    var pid = dbbox.get('person_id');
    var barangPesanan = c.keranjangFm;
    var jenisPengiriman = c.pilihanKurirFm.value;
    var metodeBayar = metodeBayarOK.value;

    var latitude = c.latitude.value;
    var longitude = c.longitude.value;
    var idOutlet = c.idOutletpadakeranjangFm.value;
    var ongkir = c.ongkirFm.value;
    var alamat = c.alamatKirimFm.value;
    var user = dbbox.get('loginSebagai');
    ...
    var url = Uri.parse(
        '${c.baseURLfreshmart}/mobileAppsUser/kirimOrderFreshmartSaldoV2');
  ...
  void kirimBelanjaCOD() async {
    ...
    var token = dbbox.get('token');
    var pid = dbbox.get('person_id');
    var barangPesanan = jsonEncode(c.keranjangFm);
    var jenisPengiriman = c.pilihanKurirFm.value;
    var metodeBayar = metodeBayarOK.value;
    var latitude = c.latitude.value;
    var longitude = c.longitude.value;
    ...
    var url = Uri.parse(
        '${c.baseURLfreshmart}/mobileAppsUser/kirimOrderFreshmartCODV2');
    ...
    if (response.statusCode == 200) {
      Map<String, dynamic> hasil = jsonDecode(response.body);
      if (hasil['status'] == 'success') {
        c.iddriverFm.value = hasil['idDriver'];
        c.kodetransaksiFm.value = hasil['kodetrx'];
  ...
  void prosesPembayaranPaymentSaldo(pin) async {
    ...
    var url = Uri.parse('${c.baseURL}/mobileApps/paymentFreshmart');
