...
class PembayaranPerjalanan extends StatefulWidget {
  @override
  _PembayaranPerjalananState createState() => _PembayaranPerjalananState();
}

class _PembayaranPerjalananState extends State<PembayaranPerjalanan> {
  ...
  void pilihMetodeBayar(String pilih, newVal) {
    if (pilih == 'saldo') {
      if (c.saldoInt.value >= c.hargaIntPerjalananMotor.value) {
        if (newVal == true) {
  ...
  void cekOngkosDanDriverTersedia() async {
    ...
    var token = dbbox.get('token');
    var pid = dbbox.get('person_id');
    var latTujuan = c.latTujuan.value;
    var longTujuan = c.longTujuan.value;
    var latJemput = c.latJemput.value;
    var longJemput = c.longJemput.value;
    var ketJemput = c.ketJemput.value;
    var ketTujuan = c.ketTujuan.value;
    var user = dbbox.get('loginSebagai');
    ...
    var url = Uri.parse('${c.baseURLdriver}/mobileAppsUser/cekDriver');
    ...
    if (response.statusCode == 200) {
      Map<String, dynamic> hasil = jsonDecode(response.body);
      if (hasil['status'] == 'success') {
        c.alamatTujuan.value = hasil['destination'];
        c.alamatJemput.value = hasil['origin'];
        jarak.value = hasil['jarakKm'];
        waktu.value = hasil['waktuTempuh'];
        c.hargaPerjalananMotor.value = hasil['hargaRp'];
        c.hargaIntPerjalananMotor.value = hasil['hargaInt'];
  ...
  void lakukanPemesananCOD() async {
    ...
    var url = Uri.parse('${c.baseURLdriver}/mobileAppsUser/pesanMotorCOD_v2');
    ...
    if (response.statusCode == 200) {
      ...
      if (hasil['status'] == 'success') {
        ...
        for (var a = 0; a < driverList.length; a++) {
          c.arrIdDriver.add(driverList[a]);
        }
        Get.offAll(Dashboardku());
  ...
  requestPembayaranViaSaldo() async {
    ...
    var url = Uri.parse('${c.baseURLdriver}/mobileAppsUser/pesanMotorSaldo_v2');
