...
import 'trackingdriverPickup.dart';

class PembayaranPerjalananPickup extends StatefulWidget {
  @override
  _PembayaranPerjalananPickupState createState() =>
      _PembayaranPerjalananPickupState();
}

class _PembayaranPerjalananPickupState
    extends State<PembayaranPerjalananPickup> {
  ...
  void pilihMetodeBayar(String pilih, newVal) {
    if (pilih == 'saldo') {
      if (c.saldoInt.value >= c.hargaIntPerjalananPickup.value) {
        if (newVal == true) {
  ...
  void cekOngkosDanDriverTersedia() async {
    ...
    var token = dbbox.get('token');
    var pid = dbbox.get('person_id');
    var latTujuan = c.latTujuanPickup.value;
    var longTujuan = c.longTujuanPickup.value;
    var latJemput = c.latJemputPickup.value;
    var longJemput = c.longJemputPickup.value;
    var ketJemput = c.ketJemputPickup.value;
    var ketTujuan = c.ketTujuanPickup.value;
    var user = dbbox.get('loginSebagai');
    ...
    var url = Uri.parse('${c.baseURLdriver}/mobileAppsUser/cekDriverPickup');
    ...
    if (response.statusCode == 200) {
      Map<String, dynamic> hasil = jsonDecode(response.body);
      if (hasil['status'] == 'success') {
        c.alamatTujuanPickup.value = hasil['destination'];
        c.alamatJemputPickup.value = hasil['origin'];
        jarak.value = hasil['jarakKm'];
        waktu.value = hasil['waktuTempuh'];
        c.hargaPerjalananPickup.value = hasil['hargaRp'];
        c.hargaIntPerjalananPickup.value = hasil['hargaInt'];
  ...
  void lakukanPemesananCOD() async {
    ...
    var url =
        Uri.parse('${c.baseURLdriver}/mobileAppsUser/pesanMotorCODPickupV2');
    ...
    if (response.statusCode == 200) {
      ...
      if (hasil['status'] == 'success') {
        ...
        c.kodeTransaksiPickup.value = hasil['kodetrx'];
        c.arrIdDriverPickup.clear();
        for (var a = 0; a < driverList.length; a++) {
          c.arrIdDriverPickup.add(driverList[a]);
        }
        Get.offAll(Dashboardku());
  ...
  requestPembayaranViaSaldo() async {
    ...
    var url =
        Uri.parse('${c.baseURLdriver}/mobileAppsUser/pesanMotorSaldoPickupV2');
